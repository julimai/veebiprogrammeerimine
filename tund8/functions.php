...
function readAllIdeas(){
	...
	$stmt=$mysqli->prepare("SELECT id, idea, ideacolor FROM userideas WHERE userid =? AND deleted is NULL ORDER BY id DESC"); //uusim mõte võiks olla eespool
	...
	//Lisame lingi:  | <a href="edituserides.php?id=6">Toimeta</a> 
	}	
	$stmt->close();
	$mysqli->close();
	return $ideas;
